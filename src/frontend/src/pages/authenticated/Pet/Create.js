...
export default function Create() {
  ...
  const defaultValues = {
    is_microchipped: 0,
    is_disabled: 0,
    vaccines: [{ clinic_id: null, vaccine: '', last_vaccinated_at: '' }],
    dewormers: [{ clinic_id: null, dewormer: '', last_dewormed_at: '' }],
  };
  ...
  const [vaccines, setVaccines] = useState(defaultValues.vaccines);
  const [dewormers, setDewormers] = useState(defaultValues.dewormers);
  ...
  useEffect(() => {
    ...
    // details
    setValue('details.age', '');
    setValue('details.birthdate', '');
    setValue('details.coat', '');
    setValue('details.pattern', '');
    setValue('details.weight', '');
    setValue('details.last_weighed_at', '');
    setValue('details.is_disabled', '');
    // vaccination
    setValue('vaccines', []);
    // deworming
    setValue('dewormers', []);
  ...
  const fetchSpecies = async () => {
    try {
      ...
      console.log('error in fetchSpecies');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchClinics = async () => {
    try {
      ...
      console.log('error in fetchClinics');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleIsDisabledRadioGroupOnChange = (e) => {
    const { value } = e.target;
    setIsDisabled(value);
    setValue('details.is_disabled', value);
  };
  ...
  const handleBirthdateOnChange = (value) => {
    ...
    setValue('details.birthdate', date, { shouldValidate: true });
  };
